...
**Startup order matters.** `main()` (`lib/main.dart`) must `await SharedPreferenceService().init()` and `await WordleService().init()` *before* `runApp`, because both are singletons read synchronously during the first widget build (dark-mode theme, today's word, rehydrated grid). `NotificationService().init()` is **not** awaited here: it loads the full timezone database and makes platform-channel calls that nothing on the startup screen needs, so it is scheduled in a post-first-frame callback to keep it off the first-frame critical path (scheduling reminders still only happens when the user opts in). Adding new persisted state that the first frame depends on means initializing it (awaited) here too. (Note: startup profiling shows `SharedPreferenceService().init()` itself dominates time-to-first-frame on cold start — see the auto-memory note.)
...
**Reveal animation finalization & lifecycle.** The per-tile flip (`tile.dart`) and the staggered win-row bounce (`animations/bounce.dart`) start via `Future.delayed`, so their callbacks must guard `mounted` before driving the `AnimationController` — otherwise leaving the board mid-animation throws `AnimationController.forward() called after dispose()`. When the flip completes, `Tile` finalizes the board from the **current** `gridProvider` state (read fresh, not the value captured when the flip was scheduled), so letters typed during the animation survive; only already-revealed (non-`initial`) tiles are marked `hasFlipAnimationPlayed`.
...
**Notifications (Android).** `NotificationService` (`lib/util/notification_service.dart`, a singleton) wraps `flutter_local_notifications` for the opt-in daily reminder (off by default). It uses timezone-aware scheduling with a **hybrid exact/inexact** mode (`_scheduleMode()`): if exact alarms are permitted (`canScheduleExactNotifications`) it schedules to-the-minute (`exactAllowWhileIdle`), otherwise it falls back to `inexactAllowWhileIdle` so the reminder still works without the special permission. Enabling the reminder best-effort requests the exact-alarm permission (`requestExactAlarmPermission`, opens system settings on Android 12+). The reminder is a **one-shot, re-armed** — *not* an OS-level daily repeat (`matchDateTimeComponents`), because the plugin always fires a repeat at the next occurrence of the time and so cannot skip individual days. Instead, the next single reminder is (re)scheduled when the reminder is enabled, when its time changes, and after each completed daily game (`Grid.recordOutcome` → `_skipReminderForToday`, localising the text via `lookupAppLocalizations` since there is no `BuildContext`). To **skip days already played**, completing the daily writes `SharedPreferencesKeys.lastPlayedDate`; scheduling passes `skipToday` (= `hasPlayedDailyToday()` from the settings paths, always `true` from `recordOutcome`) so the next reminder lands tomorrow. Consequence: the reminder keeps nudging daily only while the player opens the app / plays between reminders; it is not guaranteed for a user who never reopens the app. The notification carries a monochrome white-owl status-bar icon (`ic_stat_reminder`, generated from the launcher foreground) and the full-color logo as the large icon (`ic_notification_large`, copied from the launcher raster into `res/drawable-*` because the plugin only resolves bitmaps from `drawable`, not `mipmap`). Because both icons are referenced **only by name string at runtime** (never via `R.drawable`), Flutter's release build (R8 + `shrinkResources`) marks them unreachable and strips them — so `res/raw/keep.xml` (`tools:keep`) must retain both, otherwise `scheduleDailyReminder` throws `PlatformException(invalid_icon)` in **release only** and the reminder toggle silently fails (debug builds don't shrink, so this never reproduces there; guarded by `test/unit/android_manifest_test.dart`). As a backstop, `ReminderSetting._setEnabled` wraps scheduling in a try/catch and surfaces a snackbar (`reminderError`) instead of leaving the toggle stuck off. The underlying plugin is injectable via `pluginForTesting`. The `ReminderSetting` widget owns the toggle + time picker and localizes the notification text at schedule time; the chosen time is held in `ReminderTimeNotifier` (`lib/riverpod/reminder_time/reminder_time_notifier.dart`), persisted as an `HH:mm` string under `SharedPreferencesKeys.reminderTime` (default 8 PM), so the picker reflects a new time immediately instead of only after a restart. Native requirements: core library desugaring in `android/app/build.gradle`, `res/raw/keep.xml` to keep the notification icons through release resource shrinking, the `POST_NOTIFICATIONS`/`RECEIVE_BOOT_COMPLETED`/`SCHEDULE_EXACT_ALARM` manifest permissions, and the plugin's broadcast receivers (`ScheduledNotificationReceiver`, `ScheduledNotificationBootReceiver`, `ActionBroadcastReceiver`) declared in `AndroidManifest.xml` — the v19 plugin does **not** declare them itself, and without `ScheduledNotificationReceiver` the alarm fires but no notification is ever posted (`test/unit/android_manifest_test.dart` guards this).
...
Releases are tag-driven via GitHub Actions (`.github/workflows/release.yml`). The single trigger is pushing a **semver tag** matching `*.*.*` (e.g. `2.1.0`, **no `v` prefix**). Two jobs run:

- **`changelog`** — checks out `main` (full history), then a bash step reconstructs a new `CHANGELOG.md` section from `git log <prev-tag>..<new-tag>`, bucketing commit subjects by conventional-commit type (`feat`→Added, `fix`→Fixed, `perf`→Performance, `docs`→Documentation, `refactor`/`i18n`/`style`/`build`/unprefixed→Changed; `chore`/`ci`/`test` dropped). It inserts the section directly **below the `<!-- BEGIN_VERSIONS -->` marker** in `CHANGELOG.md` and commits it back to `main` with `[skip ci]`. The step is idempotent (greps for an existing `## <tag>` header first). **The CHANGELOG section header format is `## <version>` with no brackets — if you change that style, update both the marker-insertion code and the idempotency grep in the workflow to match, or auto-updates will duplicate or mis-detect entries.**
- **`build`** — checks out the tag, sets up Java 17 + Flutter **pinned to `3.41.6`**, runs `build_runner`, reconstructs the release keystore from secrets, builds a single universal signed APK (`flutter build apk --release`), and publishes a GitHub Release (auto-generated notes) with `red_owl-<tag>.apk`.

Signing in CI needs four repo secrets: `KEYSTORE_BASE64` (base64 of `android/app/upload-keystore.jks`), `KEYSTORE_PASSWORD`, `KEY_PASSWORD`, `KEY_ALIAS`. The keystore and `android/key.properties` are gitignored (never in the repo); the `build` job writes `android/key.properties` from those secrets at build time. The tag is **not** validated against `pubspec.yaml`'s version — keep them in sync by hand. `CHANGELOG.md` is the user-facing history; the README's Changelog section is just a pointer to it.
